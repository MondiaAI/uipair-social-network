  ...
  const load = async () => {
    ...
    const [{ data: projectData }, memberQ] = await Promise.all([
      supabase
        .from("projects")
        .select(
          "id, name, description, subject, custom_subject, category, custom_category, open_roles, team_size_limit, member_count, deadline, is_public, join_fee_cents, fee_interval, progress, creator_id, updated_at",
        )
